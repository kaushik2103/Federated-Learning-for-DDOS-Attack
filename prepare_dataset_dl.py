...
def prepare_dataset_for_dl(num_clients=5, num_features=50, num_samples=20000, num_classes=5, alpha=50.0):
    print(f"Generating and preparing dataset for Deep Learning (alpha={alpha})...")
    ...
    print("Processing and saving data for each client...")
    for client_id in range(num_clients):
        ...
        X_train_final = np.transpose(X_train_reshaped, (0, 2, 1))
        X_test_final = np.transpose(X_test_reshaped, (0, 2, 1))

        dir_path = f"client_{client_id + 1}/processed-dataset-dl"
        os.makedirs(dir_path, exist_ok=True)

        np.save(os.path.join(dir_path, "x_train.npy"), X_train_final)
        np.save(os.path.join(dir_path, "y_train.npy"), y_train.values)
        np.save(os.path.join(dir_path, "x_test.npy"), X_test_final)
        np.save(os.path.join(dir_path, "y_test.npy"), y_test.values)

        print(
            f"Client {client_id + 1} DL data saved. Train shape: {X_train_final.shape}, Test shape: {X_test_final.shape}")
